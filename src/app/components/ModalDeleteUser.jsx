"use client"

import Box from '@mui/material/Box'
import Button from '@mui/material/Button'
import Typography from '@mui/material/Typography'
import Modal from '@mui/material/Modal'
import { useState } from 'react'
import { LinearProgress } from '@mui/material'
import { useSession } from 'next-auth/react'
import { getUsers } from '../(private)/lista/usuario/page'

const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
    transform: 'translate(-50%, -50%)',
    width: 400,
    bgcolor: 'background.paper',
    borderRadius: '10px',
    boxShadow: 24,
    p: 4,
}

export const ModalDeleteUser = ({ setOpen, open, uid, setOpenSnakbar, setOpenSnakbarError }) => {

    const { data: session } = useSession()
    let token = ''

    if (session && session.user && session.user.token) {
        token = session.user.token
    }

    const [buttonDisable, setButtonDisable] = useState(false)

    const handleClose = () => setOpen(false)

    const handleSubmit = async () => {

        setButtonDisable(true)

        const myHeaders = new Headers();
        myHeaders.append("x-token", token)

        const requestOptions = {
            method: "DELETE",
            headers: myHeaders,
            redirect: "follow"
        }

        try {

            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${uid}`, requestOptions)

            setOpen(false)
            setButtonDisable(false)
            if(res.ok){
                setOpenSnakbar(true)
            }else{
                setOpenSnakbarError(true)
            }

            setTimeout(() => {
                setOpenSnakbar(false)
                setOpenSnakbarError(false)
            }, 3000)

            location.reload()

        } catch (error) {
            setButtonDisable(false)
            throw error

        }
    }

    return (
        <div>
            <Modal
                open={open}
                onClose={handleClose}
                aria-labelledby="modal-modal-title"
                aria-describedby="modal-modal-description"
            >
                <Box sx={style}>
                    <Typography sx={{ textAlign: 'center' }} id="modal-modal-title">
                        ¿Está seguro eliminar este usuario?
                    </Typography>

                    <Box
                        sx={{
                            display: 'flex',
                            justifyContent: 'center',
                            marginTop: '20px'
                        }}
                    >
                        <Button disabled={buttonDisable} onClick={handleSubmit} sx={{marginRight: '5px'}} variant="contained" color="primary">Si</Button>
                        <Button disabled={buttonDisable} onClick={handleClose} sx={{marginLeft: '5px'}} variant="outlined" color="primary">No</Button>
                    </Box>

                </Box>


            </Modal>

            {/* Spinner */}
            <Box 
                sx={{    
                    position: 'fixed',
                    bottom: 0,
                    left: 0,
                    width: '100%',
                    zIndex: '9999'
                }}
            >
                {
                    buttonDisable && <LinearProgress />
                }  
            </Box>
        </div>
    )
}